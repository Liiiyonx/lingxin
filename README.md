...
> 面向高校辅导员、学工处与学生的多模态心理辅助平台。以通义千问大模型为智能中枢，融合多模态情绪识别与知识检索能力，把「谈心记录、视频情绪识别、风险预警、危机工单、AI 数字人、预约咨询」串成一条可追溯的闭环，让每一次学生状态变化都有证据、有通知、有处置。

## 关键指标

- 文本情绪识别：准确率 **73.33%**、宏平均 F1 **77.56%**（30 条内置人工标注评测集，生产关键词基线）
- **80 项**自动化测试全部通过，**61 项**账户与数据完整性校验全部通过
- 已部署至云服务器上线运行（Vue3 + Flask + SQLite + ChromaDB）
