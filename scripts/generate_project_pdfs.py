...
def build_overview():
    ...
    story = [
        ...
        _bullet("70 项自动化测试全部通过，61 项账号与数据校验全部通过；文本情绪八分类评测样本 16 条，准确率与宏平均 F1 均为 100%。"),
...
def build_design_doc():
    ...
    story = [
        ...
        _bullet("自动化测试：python -m pytest tests/ -q，结果为 70 passed、0 failed。"),
        ...
        _body(
            ...
            "并形成可复现的演示数据、70/70 自动化测试和 61/61 数据校验基线，具备进入试点应用和后续扩展的基础。"
        ),
        Spacer(1, 0.2 * cm),
    ]
    _doc(
